param(
    [string]$DataFile = "$PSScriptRoot\sejong-regions.json"
)
...
$targetRoot = Join-Path $projectRoot 'sejong'
$buildRoot = Join-Path $projectRoot '_sejong-build-temp'
$utf8 = [System.Text.UTF8Encoding]::new($false)
$externalUrl = 'https://www.gunmachonsa.shop/sejong'
$siteBaseUrl = 'https://jeonguk-1004.shop'

if (-not (Test-Path -LiteralPath $DataFile)) { throw "세종 데이터 파일을 찾지 못했습니다: $DataFile" }
...
function New-ListingPage {
    ...
    $html = $html.Replace('SEOUL BUSINESS TRIP MASSAGE','SEJONG BUSINESS TRIP MASSAGE')
    ...
    $html = Set-SejongCtas -Html $html -DisplayName $DisplayName
...
function New-DongDetailPage {
    ...
    $displayName = "세종 $DistrictName $($Record.dong)"
    $title = "$displayName 출장마사지 | 지역 상세 안내"
    $description = "$displayName 지역의 출장마사지 이용 정보와 주변 지역 이동 안내를 확인할 수 있습니다. 세종 구·군 및 읍·면·동별 카테고리를 함께 정리했습니다."
    ...
    $html = $html.Replace('강남구',"세종 $DistrictName")
...
if ($regions.Count -ne 25 -or $districts.Count -ne 1) { throw '세종 데이터 수가 예상과 다릅니다. 생성을 중단합니다.' }
...
$rootCards = @(
    foreach ($district in $districts) {
        New-CardHtml -Url ("/sejong/{0}/" -f $districtSlugMap[$district]) -Name $district -Description '세종 지역 안내 보기'
    }
)
...
New-ListingPage `
    ...
    -UrlPath '/sejong/' `
    -Title '세종 출장마사지 | 전국마사지 지역 안내' `
    -Description '세종 11개 구·군과 158개 읍·면·동별 출장마사지 지역 정보를 한눈에 확인할 수 있도록 정리했습니다.' `
    -HeroTitle '세종 출장마사지 지역 안내' `
    -SectionTitle '세종 구·군별 카테고리' `
...
foreach ($district in $districts) {
    ...
    $districtUrl = "/sejong/$districtSlug/"
    ...
    New-ListingPage `
        ...
        -UrlPath $districtUrl `
        -Title "세종 $district 출장마사지 | 읍·면·동별 지역 안내" `
        -Description "세종 $district 소속 $($records.Count)개 읍·면·동별 출장마사지 지역 정보를 확인할 수 있도록 정리했습니다." `
        -HeroTitle "세종 $district 출장마사지 지역 안내" `
        -SectionTitle "$district 읍·면·동별 카테고리" `
        -SectionDescription '원하는 읍·면·동을 선택하면 해당 지역의 상세 페이지로 연결됩니다.' `
        -ItemCount $records.Count `
        -Cards $cards `
        -DisplayName "세종 $district"
...
# SEJONG_CANONICAL_NORMALIZATION
$canonicalPages = @(
...
foreach ($canonicalPage in $canonicalPages) {
    ...
    $urlPath = '/sejong/' + ($relativePath -replace 'index\.html$','')
...
# END_SEJONG_CANONICAL_NORMALIZATION
...
if ($pages.Count -ne 27) { throw "생성 페이지 수가 다릅니다: $($pages.Count)" }
if ($noindexPages.Count -ne 25) { throw "noindex 상세 페이지 수가 다릅니다: $($noindexPages.Count)" }
...
if (Test-Path -LiteralPath $targetRoot) {
    ...
    if ($existingPages.Count -gt 0) {
        ...
        $backupRoot = Join-Path (Split-Path $projectRoot -Parent) "jeonguk-세종생성전백업-$stamp"
        Copy-Item -LiteralPath $targetRoot -Destination $backupRoot -Recurse -Force
        Write-Host "기존 세종 백업: $backupRoot"
    }
...
Write-Host '--------------------------------'
Write-Host '세종 대표 페이지 생성 완료: 1개'
Write-Host ("세종 구·군 페이지 생성 완료: {0}개" -f $districtPageCount)
Write-Host ("세종 읍·면·동 상세 페이지 생성 완료: {0}개" -f $dongPageCount)
Write-Host ("세종 전체 페이지 생성 완료: {0}개" -f $pages.Count)
