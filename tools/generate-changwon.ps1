param(
    [string]$DataFile = "$PSScriptRoot\changwon-regions.json"
)
...
$targetRoot = Join-Path $projectRoot 'changwon'
$buildRoot = Join-Path $projectRoot '_changwon-build-temp'
$utf8 = [System.Text.UTF8Encoding]::new($false)
$externalUrl = 'https://www.gunmachonsa.shop/changwon'
$siteBaseUrl = 'https://jeongukmassage.shop'

if (-not (Test-Path -LiteralPath $DataFile)) { throw "창원 데이터 파일을 찾지 못했습니다: $DataFile" }
...
function New-ListingPage {
    ...
    $html = $html.Replace('SEOUL BUSINESS TRIP MASSAGE','CHANGWON BUSINESS TRIP MASSAGE')
    ...
    $html = Set-ChangwonCtas -Html $html -DisplayName $DisplayName
...
function New-DongDetailPage {
    ...
    $displayName = "창원 $DistrictName $($Record.dong)"
    $title = "$displayName 출장마사지 | 지역 상세 안내"
    $description = "$displayName 지역의 출장마사지 이용 정보와 주변 지역 이동 안내를 확인할 수 있습니다. 창원 구·군 및 읍·면·동별 카테고리를 함께 정리했습니다."
    ...
    $html = $html.Replace('강남구',"창원 $DistrictName")
...
if ($regions.Count -ne 55 -or $districts.Count -ne 5) { throw '창원 데이터 수가 예상과 다릅니다. 생성을 중단합니다.' }
...
$rootCards = @(
    foreach ($district in $districts) {
        New-CardHtml -Url ("/changwon/{0}/" -f $districtSlugMap[$district]) -Name $district -Description '창원 지역 안내 보기'
    }
)
...
New-ListingPage `
    ...
    -UrlPath '/changwon/' `
    -Title '창원 출장마사지 | 전국마사지 지역 안내' `
    -Description '창원 11개 구·군과 158개 읍·면·동별 출장마사지 지역 정보를 한눈에 확인할 수 있도록 정리했습니다.' `
    -HeroTitle '창원 출장마사지 지역 안내' `
    -SectionTitle '창원 구·군별 카테고리' `
...
foreach ($district in $districts) {
    ...
    $districtUrl = "/changwon/$districtSlug/"
    ...
    New-ListingPage `
        ...
        -UrlPath $districtUrl `
        -Title "창원 $district 출장마사지 | 읍·면·동별 지역 안내" `
        -Description "창원 $district 소속 $($records.Count)개 읍·면·동별 출장마사지 지역 정보를 확인할 수 있도록 정리했습니다." `
        -HeroTitle "창원 $district 출장마사지 지역 안내" `
        -SectionTitle "$district 읍·면·동별 카테고리" `
        -SectionDescription '원하는 읍·면·동을 선택하면 해당 지역의 상세 페이지로 연결됩니다.' `
        -ItemCount $records.Count `
        -Cards $cards `
        -DisplayName "창원 $district"
...
# CHANGWON_CANONICAL_NORMALIZATION
$canonicalPages = @(
...
foreach ($canonicalPage in $canonicalPages) {
    ...
    $urlPath = '/changwon/' + ($relativePath -replace 'index\.html$','')
...
# END_CHANGWON_CANONICAL_NORMALIZATION
...
if ($pages.Count -ne 61) { throw "생성 페이지 수가 다릅니다: $($pages.Count)" }
if ($noindexPages.Count -ne 55) { throw "noindex 상세 페이지 수가 다릅니다: $($noindexPages.Count)" }
...
if (Test-Path -LiteralPath $targetRoot) {
    ...
    if ($existingPages.Count -gt 0) {
        ...
        $backupRoot = Join-Path (Split-Path $projectRoot -Parent) "jeonguk-창원생성전백업-$stamp"
        Copy-Item -LiteralPath $targetRoot -Destination $backupRoot -Recurse -Force
        Write-Host "기존 창원 백업: $backupRoot"
    }
...
Write-Host '--------------------------------'
Write-Host '창원 대표 페이지 생성 완료: 1개'
Write-Host ("창원 구·군 페이지 생성 완료: {0}개" -f $districtPageCount)
Write-Host ("창원 읍·면·동 상세 페이지 생성 완료: {0}개" -f $dongPageCount)
Write-Host ("창원 전체 페이지 생성 완료: {0}개" -f $pages.Count)
