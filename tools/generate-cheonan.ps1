param(
    [string]$DataFile = "$PSScriptRoot\cheonan-regions.json"
)
...
$targetRoot = Join-Path $projectRoot 'cheonan'
$buildRoot = Join-Path $projectRoot '_cheonan-build-temp'
$utf8 = [System.Text.UTF8Encoding]::new($false)
$externalUrl = 'https://www.gunmachonsa.shop/cheonan'
$siteBaseUrl = 'https://jeongukmassage.shop'

if (-not (Test-Path -LiteralPath $DataFile)) { throw "천안 데이터 파일을 찾지 못했습니다: $DataFile" }
...
function New-ListingPage {
    ...
    $html = $html.Replace('SEOUL BUSINESS TRIP MASSAGE','CHEONAN BUSINESS TRIP MASSAGE')
    ...
    $html = Set-CheonanCtas -Html $html -DisplayName $DisplayName
...
function New-DongDetailPage {
    ...
    $displayName = "천안 $DistrictName $($Record.dong)"
    $title = "$displayName 출장마사지 | 지역 상세 안내"
    $description = "$displayName 지역의 출장마사지 이용 정보와 주변 지역 이동 안내를 확인할 수 있습니다. 천안 구·군 및 읍·면·동별 카테고리를 함께 정리했습니다."
    ...
    $html = $html.Replace('강남구',"천안 $DistrictName")
...
if ($regions.Count -ne 31 -or $districts.Count -ne 2) { throw '천안 데이터 수가 예상과 다릅니다. 생성을 중단합니다.' }
...
$rootCards = @(
    foreach ($district in $districts) {
        New-CardHtml -Url ("/cheonan/{0}/" -f $districtSlugMap[$district]) -Name $district -Description '천안 지역 안내 보기'
    }
)
...
New-ListingPage `
    ...
    -UrlPath '/cheonan/' `
    -Title '천안 출장마사지 | 전국마사지 지역 안내' `
    -Description '천안 11개 구·군과 158개 읍·면·동별 출장마사지 지역 정보를 한눈에 확인할 수 있도록 정리했습니다.' `
    -HeroTitle '천안 출장마사지 지역 안내' `
    -SectionTitle '천안 구·군별 카테고리' `
...
foreach ($district in $districts) {
    ...
    $districtUrl = "/cheonan/$districtSlug/"
    ...
    New-ListingPage `
        ...
        -UrlPath $districtUrl `
        -Title "천안 $district 출장마사지 | 읍·면·동별 지역 안내" `
        -Description "천안 $district 소속 $($records.Count)개 읍·면·동별 출장마사지 지역 정보를 확인할 수 있도록 정리했습니다." `
        -HeroTitle "천안 $district 출장마사지 지역 안내" `
        -SectionTitle "$district 읍·면·동별 카테고리" `
        -SectionDescription '원하는 읍·면·동을 선택하면 해당 지역의 상세 페이지로 연결됩니다.' `
        -ItemCount $records.Count `
        -Cards $cards `
        -DisplayName "천안 $district"
...
# CHEONAN_CANONICAL_NORMALIZATION
$canonicalPages = @(
...
foreach ($canonicalPage in $canonicalPages) {
    ...
    $urlPath = '/cheonan/' + ($relativePath -replace 'index\.html$','')
...
# END_CHEONAN_CANONICAL_NORMALIZATION
...
if ($pages.Count -ne 34) { throw "생성 페이지 수가 다릅니다: $($pages.Count)" }
if ($noindexPages.Count -ne 31) { throw "noindex 상세 페이지 수가 다릅니다: $($noindexPages.Count)" }
...
if (Test-Path -LiteralPath $targetRoot) {
    ...
    if ($existingPages.Count -gt 0) {
        ...
        $backupRoot = Join-Path (Split-Path $projectRoot -Parent) "jeonguk-천안생성전백업-$stamp"
        Copy-Item -LiteralPath $targetRoot -Destination $backupRoot -Recurse -Force
        Write-Host "기존 천안 백업: $backupRoot"
    }
...
Write-Host '--------------------------------'
Write-Host '천안 대표 페이지 생성 완료: 1개'
Write-Host ("천안 구·군 페이지 생성 완료: {0}개" -f $districtPageCount)
Write-Host ("천안 읍·면·동 상세 페이지 생성 완료: {0}개" -f $dongPageCount)
Write-Host ("천안 전체 페이지 생성 완료: {0}개" -f $pages.Count)
