...
function New-ListingPage {
    ...
    $html = [System.IO.File]::ReadAllText($seoulTemplate)
...
function New-DongDetailPage {
    ...
    $detailHtml = [System.IO.File]::ReadAllText($detailTemplate)
...
$regionList = [System.Collections.Generic.List[object]]::new()

foreach ($item in $rawRegions) {
    [void]$regionList.Add($item)
}

$regions = @(
    $regionList | Sort-Object municipality, district, dong
)
