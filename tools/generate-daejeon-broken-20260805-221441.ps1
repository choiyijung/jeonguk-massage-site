...
if ($regions.Count -ne 158 -or $districts.Count -ne 11) { throw '대전 데이터 수가 예상과 다릅니다. 생성을 중단합니다.' }
...
if ($pages.Count -ne 170) { throw "생성 페이지 수가 다릅니다: $($pages.Count)" }
if ($noindexPages.Count -ne 158) { throw "noindex 상세 페이지 수가 다릅니다: $($noindexPages.Count)" }
