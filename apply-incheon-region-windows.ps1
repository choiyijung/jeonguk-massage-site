...
foreach ($info in $targetInfos) {
    $html = $info.Html
    $label = if ($info.Service -eq 'swedish') { '전국마사지 인천 스웨디시 안내' } else { '전국마사지 인천 지역 안내' }
    ...
    if ($info.PageType -eq 'incheon') {
        ...
        $description = "전국마사지에서 $($info.DisplayName) 소속 읍·면·동별 $service 정보를 간편하게 확인하세요. 세부 지역 선택과 주변 카테고리를 보기 쉽게 정리했습니다."
    } else {
        $description = "전국마사지에서 $($info.DisplayName) 생활권의 $service 정보를 한눈에 확인하세요. 주변 지역 이동과 상세 카테고리를 보기 쉽게 정리했습니다."
    }
